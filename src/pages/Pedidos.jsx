...
const Pedidos = () => {
  ...
  /* ─────────────────────────────────────────
     PROCESAR PAGO DE MERCADO PAGO (URL PARAMS)
  ───────────────────────────────────────── */

  useEffect(() => {
    if (authLoading || !user?.id_cliente) return;

    // Detectar parámetros de redirección de Mercado Pago en la URL
    ...
    if (paymentId && mpStatus === 'approved' && externalRef) {
      ...
      handlePaymentWithParams(paymentId, externalRef);
    }
  }, [user, authLoading, searchParams]);

  // Confirmar pago cuando tenemos los params de MP en la URL
  const handlePaymentWithParams = async (paymentId, externalRef) => {
    if (confirmingPayment) return; // Prevenir doble ejecución
    try {
      setConfirmingPayment(true);
      toast.loading("Confirmando tu pago...", { id: "mp-payment" });
      ...
      console.log("[Pedidos] ✅ Respuesta:", response.data);
      toast.success("¡Pago confirmado con éxito!", { id: "mp-payment" });
      localStorage.removeItem('pending_checkout');
      await fetchOrders();
    } catch (error) {
      console.error("[Pedidos] ❌ Error:", error?.response?.data || error.message);
      // Si falla la confirmación manual, el hook global lo reintentará via verificar-checkout
      toast.error("Error al confirmar, se reintentará automáticamente", { id: "mp-payment" });
    } finally {
      ...
      const allOrders = res.data.map((order) => {
        const items = order.items.map((item) => {
          ...
          return {
            ...
            price: parseFloat(item.subtotal || 0) / item.cantidad,
          };
        });
        ...
        const statusInfo = statusMap[order.estado] || statusMap.pendiente;
      ...
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-6">
        ...
        >
          ...
          <p className="text-gray-500 text-lg leading-relaxed mb-10">
            Accede a tus pedidos, revisa el estado en tiempo real y disfruta
  ...
  const OrderCard = ({ order }) => (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-7">
        ...
        >
          {order.statusInfo.icon} {order.statusInfo.text}
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-3xl p-6 mb-8 space-y-4">
        <div>
          <p className="text-sm text-gray-500 mb-1">Tipo de entrega</p>
        ...
        {order.costoDelivery > 0 && (
          <div>
            ...
            <p className="font-bold text-red-600">
              S/. {order.costoDelivery.toFixed(2)}
            </p>
          </div>
        )}
        {order.estimatedTime && (
          <div>
            ...
            <p className="font-bold text-gray-900">⏰ {order.estimatedTime}</p>
          </div>
        )}
      </div>
  ...
  /* ─────────────────────────────────────────
     STATS SECTION (del segundo diseño)
  ───────────────────────────────────────── */

  const totalGastado = orders.historial
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10 py-28">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-gray-300 text-lg max-w-2xl leading-relaxed">
              Sigue el estado de tus pedidos en tiempo real y administra tu
      ...
      {/* CONFIRMANDO PAGO BANNER */}
      {confirmingPayment && (
        <section className="px-6 lg:px-10 -mt-6">
          <div className="max-w-7xl mx-auto">
            <div className="bg-green-100 text-green-700 p-4 rounded-xl font-bold animate-pulse text-center">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                ...
                <div>
                  <div className="text-3xl font-black text-gray-900">{orders.activos.length}</div>
                  <div className="text-gray-500">Pedidos Activos</div>
                </div>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                <div className="text-4xl">✅</div>
                <div>
                  <div className="text-3xl font-black text-gray-900">{orders.historial.length}</div>
                  <div className="text-gray-500">Pedidos Completados</div>
                </div>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                <div className="text-4xl">💰</div>
                <div>
                  <div className="text-3xl font-black text-red-600">S/. {totalGastado.toFixed(2)}</div>
